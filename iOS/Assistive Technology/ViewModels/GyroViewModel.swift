...
protocol GyroViewModelDelegate {
...
/**
    Performs data processing of gyroscope input.
    Acts as an interface between the `GyroViewController` and services.
 */
class GyroViewModel {
    /// `GyroViewController`
    var delegate: GyroViewModelDelegate?
    /// Current processed X co-ordinate
    var currentX: CGFloat = 0
    /// Current processed Y co-ordinate
    var currentY: CGFloat = 0
    /// Co-ordinate of the up, right, down, left arrows
    var hitPoints: [CGPoint] = []
    /// x PID controller
    var xPulseController: Pulse?
    /// y PID controller
    var yPulseController: Pulse?
    /// Current value of the sensitivity bar
    var sensitivity: Float = 0.5
    
    
    /// Set up PID controllers to generate and send direction values to the server
    init() {
        // Configuration
    ...
    /// Convert raw gyroscope data to a point on the screen
    /// - Parameters:
    ///   - data: Raw gyro data input
    ///   - centre: Co-ordinates of centre point
    ///   - hitPoints: List of up, right, down, left points on screen
    ///   - sensitivity: Value of sensitivity slider
    public func handleGyroInput(data: CMDeviceMotion, centre: CGPoint, hitPoints: [CGPoint], sensitivity: Float) {
        ...
        let newX = centre.x + (CGFloat(2 * data.attitude.roll) * 300 * CGFloat(sensitivity));
        let newY = centre.y + (CGFloat(2 * data.attitude.pitch) * 300 * CGFloat(sensitivity));
                
        self.xPulseController?.setPoint = newX
        self.yPulseController?.setPoint = newY
    }
    
    /// Calculate the current direction from a point on the screen, by selecting which directional marker the central point is closest to
    /// - Parameters:
    ///   - x: X co-ordinate of the centre dot
    ///   - y: Y co-ordinate of the centre dot
    ///   - hitPoints: The co-ordinates of the up, right, down, left points to use to calculate distance
